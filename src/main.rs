...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let dataset = InMemDataset::from_csv(file_path, builder)?;

    let json_data = read_to_string("./trajectories.json")?;
    let trajectories = parse_trajectories(&json_data)?;
    println!("Parsed trajectories from file: {:#?}", trajectories);
